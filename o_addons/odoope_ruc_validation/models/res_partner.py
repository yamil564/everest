        ...
        if self.catalog_06_id and self.catalog_06_id.code == '1':
           #Valida DNI
            if self.vat and len(self.vat) != 8:
                ...
                if not d['error']:
                    d = d['data']
                    self.name = str(d['nombre'])
                    #raise Warning(d)
                    #self.name = '%s %s %s' % (d['name'],d['name'],d['name'])
            ...
            if self.vat and len(self.vat) != 11:
                ...
                # Si es HABIDO, caso contrario es NO HABIDO condicion_contribuyente
                ...
                self.name = d['nombre_o_razon_social'] != '-' and d['nombre_o_razon_social'] or d['nombre_o_razon_social']
                self.registration_name = d['nombre_o_razon_social']
                self.street = d['direccion_simple'] #domicilio_fiscal direccion
